...
----------------------------------------------------------------
module Alpha.Canonical.Structures.Choose (Choose(..),choose) where
import Alpha.Canonical.Structures.Domain
    
-- | Represents the "choose" combinatorial expression
newtype Choose a = Choose (a,a)
    deriving(Eq,Ord,Generic,Data,Typeable)

instance DomainInt a => Computable (Choose a) where
    type Computed (Choose a) = a
    compute (Choose (n,k)) = choose n k
{-
...
data Wheel = Wheel {-# UNPACK #-}!Natural ![Natural]
...
primes = seive wheels primes primeSquares
    ...
    wheels = Wheel 1 [1] : zip nextSize wheels primes
        where
